...
function Navbar() {
  ...
  return (
    ...
    <nav ref={navRef}
      className="fixed left-0 right-0 z-50 h-[72px]"
      style={{
        top: "36px",
        background: navBg,
    ...
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-10 w-full" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", height: "100%" }} dir={isAr ? "rtl" : "ltr"}>
        {/* Logo mark */}
        ...
        {/* Desktop nav */}
        <div className="hidden lg:flex items-center justify-center" style={{ flexDirection: isAr ? "row-reverse" : "row" }}>
          {NAV_LINKS_EN.map((l) => {
            ...
            if (hasChildren) {
              return (
                ...
                >
                  <a href={l.href}
                    className="relative px-2 py-2 font-semibold tracking-normal uppercase transition-colors font-outfit inline-flex items-center gap-1 whitespace-nowrap"
                    style={{ color: isActive ? activeColor : navTextBase, fontSize: "12px" }}
            ...
            return (
              <a key={l.href} href={l.href}
                className="relative px-2 py-2 font-semibold tracking-normal uppercase transition-colors font-outfit whitespace-nowrap"
                style={{ color: isActive ? activeColor : navTextBase, fontSize: "12px" }}
                onMouseEnter={e => { if (!isActive) (e.currentTarget as HTMLElement).style.color = navTextHover; }}
                onMouseLeave={e => { if (!isActive) (e.currentTarget as HTMLElement).style.color = navTextBase; }}
              >
        ...
        {/* Right actions */}
        <div className="hidden lg:flex items-center gap-2 justify-end self-center whitespace-nowrap" style={{ flexDirection: isAr ? "row-reverse" : "row" }}>
          <button onClick={toggleTheme}
            className="text-[10px] font-semibold tracking-normal uppercase cursor-pointer transition-all font-outfit px-2.5 py-1.5 rounded-full border whitespace-nowrap"
          ...
          <a href="#contact" className="btn-gold px-5 py-2.5 rounded-full text-[11px] font-bold inline-flex items-center gap-1.5 font-outfit tracking-normal whitespace-nowrap">
          ...
          <div className="flex flex-col lg:grid lg:grid-cols-[1fr_minmax(320px,480px)] gap-10 xl:gap-16 mb-8 lg:mb-10">
            ...
            <div className="lg:col-start-1 lg:row-start-1 relative z-20 overflow-visible pr-4">
              <h1 className="font-outfit font-extrabold tracking-tight mb-8 lg:mb-10 overflow-visible"
                style={{ fontSize: isAr ? "5rem" : "4rem", lineHeight: isAr ? "1.35" : "1.2" }}>
                {isAr ? (
                  <>
...
function LightboxModal({ src, alt, onClose }: { src: string; alt: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[99999] flex flex-col items-center justify-center p-2 sm:p-6 md:p-8 bg-[rgba(10,14,26,0.95)] backdrop-blur-md" onClick={onClose}>
      <div className="relative w-full max-w-[95vw] sm:max-w-2xl md:max-w-3xl h-[60vh] sm:h-[70vh] rounded-lg overflow-hidden flex items-center justify-center" onClick={(e) => e.stopPropagation()}>
        {!loaded && (
      ...
      <button
        ...
        aria-label="Close"
        onClick={onClose}
        className="mt-4 mb-2 text-white/80 hover:text-white px-6 py-2.5 rounded-full border border-white/20 hover:border-white/40 hover:bg-white/10 transition-all duration-300 cursor-pointer z-[100000] text-sm font-semibold font-outfit"
      >
        Close
      </button>
    </div>
